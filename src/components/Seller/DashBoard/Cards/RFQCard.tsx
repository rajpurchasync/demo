...
export function RFQCard({ rfq, onClick }: RFQCardProps) {
  ...
  const isOverdue =
    new Date(rfq.dueDate) < new Date() && rfq.status !== "closed";
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between text-xs">
          <div className="flex items-center text-gray-600">
            ...
            <span
              className={isOverdue ? "text-red-600 font-medium" : "font-medium"}
            >
              Due {formatDate(rfq.dueDate)}
            </span>
          </div>
          <span className="text-gray-600 font-medium">
            {rfq.suppliersInvited} supplier
            {rfq.suppliersInvited !== 1 ? "s" : ""} invited
          </span>
        </div>
      </div>
    </div>
