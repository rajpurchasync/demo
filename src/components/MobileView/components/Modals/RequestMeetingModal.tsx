...
export function RequestMeetingModal({ isOpen, onClose, supplierName }: RequestMeetingModalProps) {
  ...
  const handleSubmit = () => {
    ...
    // Add to ToDo list
    addMockToDo({
      title: `Meeting: ${formData.topic} with ${supplierName}`,
      type: 'task',
      status: 'ongoing',
      dueDate: formData.date,
      priority: 'medium',
      tags: ['meeting', supplierName.toLowerCase().replace(/\s+/g, '-')],
      description: `${formData.isOnline ? 'Online' : 'On-site'} meeting scheduled for ${formData.time}`
    });
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Toggle
              ...
              onChange={(value) => handleInputChange('isOnline', value === 'true')}
            />
          </div>
