...
export function RequestSampleModal({ isOpen, onClose, supplierName }: RequestSampleModalProps) {
  ...
  const handleSubmit = () => {
    ...
    // Add to ToDo list
    addMockToDo({
      title: `Sample request: ${formData.sampleType} from ${supplierName}`,
      type: 'sample',
      status: 'ongoing',
      dueDate: formData.expectedDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      priority: 'medium',
      tags: ['sample-request', supplierName.toLowerCase().replace(/\s+/g, '-')],
  ...
  const locationOptions = [
    ...
    ...mockLocations.map(loc => ({ value: loc.id, label: `${loc.name} - ${loc.city}, ${loc.state}` })),
      ...
      <div className="bg-white w-full max-w-md mx-auto rounded-t-2xl p-4 animate-slide-up max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-semibold text-gray-900">Request Sample</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
          ...
          {showNewLocationFields && (
            <div className="space-y-3 p-3 bg-gray-50 rounded-lg">
              <h4 className="text-sm font-medium text-gray-900">New Location Details</h4>
              <Input
                label="Street Address"
                placeholder="Enter street address"
                value={formData.newLocation.street}
                onChange={(e) => handleNewLocationChange('street', e.target.value)}
              />
              <div className="grid grid-cols-2 gap-3">
                <Input
                  ...
                  onChange={(e) => handleNewLocationChange('building', e.target.value)}
                />
                <Input
                  label="Flat/Unit"
                  placeholder="Flat number"
                  value={formData.newLocation.flat}
                  onChange={(e) => handleNewLocationChange('flat', e.target.value)}
                />
              </div>
            </div>
          )}
