...
import { X, Calendar, User, Tag, MessageSquare, CheckCircle, Edit3, AlertCircle, Send } from 'lucide-react';
  ...
  const [comments, setComments] = useState([
    {
      ...
      content: "I've started working on this task. Will need to coordinate with the suppliers by tomorrow."
    },
    {
      id: '2',
      user: 'Ahmed Hassan',
      ...
      <div className="bg-white w-full max-w-4xl h-full sm:max-h-[90vh] sm:h-auto rounded-t-lg sm:rounded-lg overflow-hidden">
        <div className="flex h-full">
          ...
          <div className="flex-1 p-3 overflow-y-auto min-w-0">
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1 min-w-0">
                {isEditing ? (
                  ...
                  <h1 className="text-sm font-semibold text-gray-900 mb-1">{todo.title}</h1>
                )}
            ...
            <div className="flex items-center space-x-2 mb-4">
              ...
              {isEditing ? (
                ...
                <div className="flex items-center text-xs">
                  <AlertCircle className={cn('w-3 h-3 mr-1', getPriorityColor())} />
                  <span className={cn('font-medium capitalize', getPriorityColor())}>
            ...
            <div className="mb-4">
              ...
              {isEditing ? (
                ...
                <div className="text-xs text-gray-900 mt-1 p-2 bg-gray-50 rounded border">
                  {editData.description || 'No description provided for this task.'}
                </div>
              )}
            </div>
            ...
            {/* Comments Section */}
            <div>
              <label className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2 block">Comments</label>
              <div className="space-y-2">
          ...
          <div className="hidden sm:block w-64 border-l border-gray-200 p-3 bg-gray-50 overflow-y-auto">
            <h3 className="text-xs font-semibold text-gray-900 uppercase tracking-wide mb-3">Activity Log</h3>
            <div className="space-y-2">
              {mockActivityLog.map((activity) => {
                const Icon = activity.icon;
                return (
                  <div key={activity.id} className="flex items-start space-x-2">
                    <div className={cn('p-1 rounded', activity.color, 'bg-opacity-10')}>
