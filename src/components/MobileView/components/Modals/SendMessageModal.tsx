...
export function SendMessageModal({ isOpen, onClose, supplierName }: SendMessageModalProps) {
  ...
  const handleSubmit = () => {
    ...
    // Add to ToDo list
    addMockToDo({
      title: `Follow up on message: ${formData.subject} to ${supplierName}`,
      type: 'task',
      status: 'ongoing',
      dueDate: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 3 days from now
      priority: 'medium',
      tags: ['message', 'follow-up', supplierName.toLowerCase().replace(/\s+/g, '-')],
