...
export function ActivityScreen() {
  ...
  const tabs = [
    ...
    { id: 'supplier', label: 'Supplier', count: mockNotifications.filter(n => n.type === 'supplier').length },
    { id: 'team', label: 'Team', count: mockNotifications.filter(n => n.type === 'team').length },
    { id: 'inbox', label: 'Inbox', count: mockNotifications.filter(n => n.type === 'inbox').length },
  ];
        ...
        {filteredNotifications.length > 0 ? (
          filteredNotifications.map((notification, index) => (
            <div
              key={notification.id}
              className={`${
                index === 0 && filteredNotifications.length > 1 ? 'rounded-t-lg' : 
                index === filteredNotifications.length - 1 || filteredNotifications.length === 1 ? 'rounded-b-lg' : 
                ''
              }`}
            >
              <NotificationCard
