...
export function ToDosScreen({
  ...
  const filteredToDos = mockToDos
    .filter((todo) => {
      ...
      // Date filter
      const matchesDateRange = (!dateFromFilter || new Date(todo.dueDate) >= new Date(dateFromFilter)) &&
        ...
        {filteredToDos.length > 0 ? (
          filteredToDos.map((todo) => (
            <div
              key={todo.id}
              className={`${
                filteredToDos.indexOf(todo) === 0 && filteredToDos.length > 1 ? 'rounded-t-lg' : 
                filteredToDos.indexOf(todo) === filteredToDos.length - 1 || filteredToDos.length === 1 ? 'rounded-b-lg' : 
                ''
              }`}
            >
              <ToDoCard
