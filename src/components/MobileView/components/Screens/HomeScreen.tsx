...
export function HomeScreen({
  ...
  const [timeFilter, setTimeFilter] = useState<"today" | "this week" | "this month" | "this year">("this week");
  ...
  const handleCardSwipe = (direction: 'left' | 'right', event: React.MouseEvent) => {
    event.stopPropagation();
    if (direction === 'right' && currentCardIndex < procurementReportCards.length - 1) {
  ...
  const cycleTimeFilter = () => {
    const filters: Array<typeof timeFilter> = ["today", "this week", "this month", "this year"];
  ...
  return (
    <div className="px-4 py-2 space-y-3 bg-gray-50 min-h-screen">
      {/* Greeting */}
      <div className="py-1">
        <h1 className="text-lg font-semibold text-gray-900">Good afternoon, Sarah!</h1>
      </div>
          ...
          return (
            ...
            >
              <div className="flex flex-col items-center space-y-1">
                <div className={`w-7 h-7 ${action.bgColor} rounded-lg flex items-center justify-center`}>
            ...
            tasks.slice(0, 2).map((task, index) => (
              <div
                ...
                className={`bg-white p-3 border border-gray-200 flex items-center space-x-3 cursor-pointer transition-colors ${
                  index === 0 && tasks.length > 1 ? 'rounded-t-lg border-b-0' : 
                  index === tasks.length - 1 || tasks.length === 1 ? 'rounded-b-lg' : 
                  'border-b-0'
                }`}
      ...
      <div className="pt-4 space-y-3">
        ...
        >
          ...
          <span className="text-sm font-medium text-gray-700">Invite Supplier</span>
        </button>
