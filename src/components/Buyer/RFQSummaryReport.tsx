...
const RFQSummaryReport: React.FC<RFQSummaryReportProps> = ({ sidebarCollapsed }) => {
  ...
  const categories = ['Office Supplies', 'Technology', 'Services', 'Manufacturing', 'Logistics'];
  ...
  const countries = ['USA', 'Canada', 'UK', 'Germany', 'France', 'Australia', 'Japan', 'India', 'China', 'Brazil'];
  ...
  const locationsByCountry: { [key: string]: string[] } = {
    ...
    'China': ['Beijing', 'Shanghai', 'Guangzhou', 'Shenzhen', 'Tianjin', 'Wuhan', 'Dongguan', 'Chengdu'],
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center space-x-3 mb-2">
              ...
              <h1 className="text-2xl lg:text-3xl font-medium text-gray-900">RFQ Summary Report</h1>
            </div>
            <p className="text-gray-600">Generate and export RFQ data reports</p>
          </div>
          <button
            ...
            <div>
              ...
              <div className="relative">
                <div className="w-full min-h-[40px] px-3 py-2 border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent">
                  ...
                  >
                    <option value="">{filters.country ? 'Select locations...' : 'Select country first'}</option>
                    {filters.country && locationsByCountry[filters.country]?.filter(loc => !filters.locations.includes(`${loc}, ${filters.country}`)).map(location => (
                ...
                <div>
                  ...
                  <div className="relative">
                    <div className="w-full min-h-[40px] px-3 py-2 border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent">
                      ...
                      <select
                        onChange={(e) => {
                          if (e.target.value && !filters.locations.includes(`${e.target.value}, ${filters.country}`)) {
                            handleLocationToggle(e.target.value);
                          }
                          e.target.value = '';
                        }}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="lg:hidden">
          <div className="p-4 border-b border-gray-200 bg-gray-50">
            <h3 className="text-lg font-medium text-gray-900">RFQ Data ({rfqData.length} records)</h3>
          </div>
              ...
              <div key={rfq.id} className="p-4">
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900 text-right">{rfq.rfqTitle}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium text-gray-600">Date:</span>
                    <span className="text-gray-900">{new Date(rfq.createdDate).toLocaleDateString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium text-gray-600">Seller:</span>
                    <span className="text-gray-900 text-right">{rfq.sellerName}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium text-gray-600">Value:</span>
                    <span className="text-gray-900 font-semibold">{rfq.rfqValue}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium text-gray-600">Quotes:</span>
                    <span className="text-gray-900">{rfq.quotationsReceived}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b-2 border-gray-200">
                <th className="text-left py-3 px-4 font-semibold text-gray-700 border-r border-gray-200">RFQ Number</th>
                <th className="text-left py-3 px-4 font-semibold text-gray-700 border-r border-gray-200">RFQ Title</th>
                <th className="text-left py-3 px-4 font-semibold text-gray-700 border-r border-gray-200">Created Date</th>
                <th className="text-left py-3 px-4 font-semibold text-gray-700 border-r border-gray-200">Seller Name</th>
                <th className="text-left py-3 px-4 font-semibold text-gray-700 border-r border-gray-200">Buyer Location</th>
                <th className="text-left py-3 px-4 font-semibold text-gray-700 border-r border-gray-200">Category</th>
                <th className="text-left py-3 px-4 font-semibold text-gray-700 border-r border-gray-200">RFQ Value</th>
                <th className="text-left py-3 px-4 font-semibold text-gray-700">Quotations Received</th>
              </tr>
            </thead>
            <tbody>
              {rfqData.map((rfq, index) => (
                <tr key={rfq.id} className={`border-b border-gray-200 hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
                  <td className="py-3 px-4 border-r border-gray-200 font-medium text-blue-600">{rfq.rfqNumber}</td>
                  <td className="py-3 px-4 border-r border-gray-200 text-gray-900">{rfq.rfqTitle}</td>
                  <td className="py-3 px-4 border-r border-gray-200 text-gray-900">{new Date(rfq.createdDate).toLocaleDateString()}</td>
                  <td className="py-3 px-4 border-r border-gray-200 text-gray-900">{rfq.sellerName}</td>
                  <td className="py-3 px-4 border-r border-gray-200 text-gray-900">{rfq.buyerLocation.city}, {rfq.buyerLocation.country}</td>
                  <td className="py-3 px-4 border-r border-gray-200 text-gray-900">{rfq.category} → {rfq.subCategory}</td>
                  <td className="py-3 px-4 border-r border-gray-200 font-semibold text-gray-900">{rfq.rfqValue}</td>
                  <td className="py-3 px-4 text-center text-gray-900">{rfq.quotationsReceived}</td>
                </tr>
              ))}
            </tbody>
          </table>
