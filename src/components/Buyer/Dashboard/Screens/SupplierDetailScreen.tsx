...
import { SupplierActionModal } from "../Modals/SupplierActionModal";
import { SupplierCompanyInfoSection } from "../Profile/SupplierCompanyInfoSection";
import { SupplierContactsSection } from "../Profile/SupplierContactsSection";
import { SupplierDocumentsSection } from "../Profile/SupplierDocumentsSection";
  ...
  const mockActivity = [
    {
      ...
      details: "Description → Added Fresh vegetables and produce supplier",
    },
    {
      id: "2",
      user: "Ahmed Hassan",
      action: "created this supplier",
      timestamp: "44 minutes ago",
      details: null,
    },
  ];

  const handleSupplierUpdate = (updates: Partial<Supplier>) => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "contact":
        return (
          <SupplierContactsSection
            ...
            onUpdate={(contacts) => handleSupplierUpdate({ contacts })}
          />
        );
      ...
      case "documents":
        return (
          <SupplierDocumentsSection
            ...
            onUpdate={(documents) => handleSupplierUpdate({ documents })}
          />
        );
      ...
      case "quotes":
        return (
          <div className="space-y-3">
            {supplierData.quotations.length > 0 ? (
              supplierData.quotations.map((quotation) => (
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-xs mt-1">
                  Quotations will appear here when available.
                </p>
              </div>
            )}
          </div>
      ...
      case "samples":
        return (
          <div className="space-y-3">
            {supplierData.samples.length > 0 ? (
              supplierData.samples.map((sample) => (
                <SupplierSampleCard
                  key={sample.id}
                  sample={sample}
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-600">
                  {supplierData.type} • {supplierData.category}
                </p>
                <p className="text-xs text-gray-500 mt-0.5">
                  {supplierData.city}, {supplierData.state}
                </p>
                <div className="flex flex-wrap gap-1 mt-2">
                  {supplierData.tags.slice(0, 3).map((tag) => (
                    <span
                      key={tag}
                  ...
                  {supplierData.tags.length > 3 && (
                    <span className="text-xs text-gray-500">
                      +{supplierData.tags.length - 3} more
                    </span>
                  )}
                </div>
              </div>
